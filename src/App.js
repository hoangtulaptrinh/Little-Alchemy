...
class App extends Component {
  ...
  addItemsToTarget = id => {
    const { Items ,targetArr} = this.state;
    this.setState({
      targetArr: [...targetArr,{
        id : id - 1,
  ...
  return (
    <div className="App">
      ...
      <h2>Drop Below Me</h2>
      <Target targetArr = {targetArr}/>
      </div>

      <div className="item-container">
              {Items.map((item, index) => (
                <Item item = {item} key={index} name={item.name} url={item.url} handleDrop={(id) => this.addItemsToTarget(id)} />
              ))}
      </div>
